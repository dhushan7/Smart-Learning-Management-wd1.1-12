...
export default function AddTask({ closeModal, onTaskAdded, editTask, isEdit, userEmail }) {
    ...
    // AI GENERATE (SECURED WITH JWT)
    const handleAIGenerate = async () => {
        if (!token) {
            return setError("Session error: Missing authentication signature.");
        }
        ...
        try {
            ...
            if (!res.ok) throw new Error("AI Endpoint refused credentials.");
    ...
    // CREATE + UPDATE (SECURED WITH JWT)
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.message || "Failed to update target task metric.");
        }
    };
